...
### D014: 强化选择系统 — 单关卡无限模式 (2026-07-14)

**Context**: 强化选择系统需要一个"下一关"才有意义。多关卡（P003）尚未决策，但可以先在单关卡内验证强化玩法。

**Decision**: 单关卡无限模式——通关后选强化 → 用强化状态重开同一关。暂不递增难度，先验证强化本身好不好玩。

**首批强化**（5 个）：
- 挡板加宽（+50%）、减速（-20%）、额外生命（+1）、多球（额外 1 球）、穿透（穿过砖块不反弹，限 N 块）

**触发时机**: 每次通关后弹出 3 选 1。

**Rationale**: 先做单关卡强化绕过 P003 的决策依赖，快速验证 Outwit 玩法。确认好玩后再决定多关卡/动态关卡方向。
